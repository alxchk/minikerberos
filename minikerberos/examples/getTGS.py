...
def main():
    ...
    parser = argparse.ArgumentParser(description='Polls the kerberos service for a TGS for the sepcified user and specified service',
                                     formatter_class=argparse.RawDescriptionHelpFormatter, epilog=KerberosCredential.help_epilog)
    parser.add_argument('kerberos_connection_string',
                        help='the kerberos target string in the following format <domain>/<username>/<secret_type>:<secret>@<domaincontroller-ip>')
    ...
    parser.add_argument(
        'ccache', help='ccache file to store the TGT ticket in')
    parser.add_argument('-u', action='store_true',
                        help='Use UDP instead of TCP (not tested)')
    parser.add_argument('-v', '--verbose', action='count', default=0)
    parser.add_argument('-L', '--linux', action='store_true', default=False)
    ...
    ccred = KerberosCredential.from_connection_string(
        args.kerberos_connection_string)
    soc_type = KerberosSocketType.UDP if args.u else KerberosSocketType.TCP
    ksoc = KerberosSocket.from_connection_string(
        args.kerberos_connection_string, soc_type)
    ...
    if not ccred.ccache:
        ...
        tgs, encpart, key = kc.get_TGS(target, is_linux=args.linux)
    else:
        ...
        for tgt, key in ccred.ccache.get_all_tgt():
            try:
                ...
                logging.debug(
                    'This ticket is not usable it seems Reason: %s' % e)
                continue
            else:
                break
