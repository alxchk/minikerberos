...
class KerberosUserEnum(object):
    ...
    def construct_tgt_req(realm, username):
        ...
        kdc_req_body['sname'] = PrincipalName(
            {'name-type': NAME_TYPE.PRINCIPAL.value, 'name-string': ['krbtgt', realm.upper()]})
        ...
        # we "support" all MS related enctypes
        kdc_req_body['etype'] = [2, 3, 16, 23, 17, 18]
    ...
    def run(self, targets, override_etype=[2, 3, 16, 23, 17, 18]):
        ...
        if not self.kcomm:
            try:
                ...
                logger.exception(
                    'Failed to get TGT ticket! Reason: %s' % str(e))

        tgss = []
        for target in targets:
            try:
                tgs, encTGSRepPart, key = self.kcomm.get_TGS(
                    target, override_etype=override_etype)
                tgss.append(tgs)
            except Exception as e:
                logger.debug('Failed to get TGS ticket for user %s/%s/%s! Reason: %s' %
                             (target.domain, str(target.service), target.username, str(e)))
                continue
