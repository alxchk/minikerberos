		...
		try:
			...
			while True:
				...
				if total_length is None:
					...
					print "RL LEN", len(response_linearized)

					total_length, = struct.unpack_from('>I', response_linearized)
				...
				print "RECV: ", total_received, "/", total_length
...
class KerbrosComm(object):
	...
	def get_TGT(self, override_etype=None, decrypt_tgt=True):
		...
		kdc_req_body['sname'] = PrincipalName({
			'name-type': NAME_TYPE.PRINCIPAL.value,
			'name-string': [
				u'krbtgt', as_str(self.usercreds.domain).upper()
			]
		})
		...
		pa_data_1 = {}
		pa_data_1['padata-type'] = int(PADATA_TYPE(u'PA-PAC-REQUEST'))
	...
	def get_TGS(self, spn_user, override_etype = None, is_linux = False):
		...
		authenticator_data['crealm'] = Realm(as_str_(self.kerberos_TGT['crealm']))
	...
	def S4U2self(self, user_to_impersonate, supp_enc_methods=SUPPORTED_METHODS):
		...
		authenticator_data = {}
		authenticator_data['authenticator-vno'] = krb5_pvno
		authenticator_data['crealm'] = Realm(as_str(self.kerberos_TGT['crealm']))
		authenticator_data['cname'] = self.kerberos_TGT['cname']
		authenticator_data['cusec'] = now.microsecond
		authenticator_data['ctime'] = now
