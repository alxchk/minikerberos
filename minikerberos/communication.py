...
from minikerberos.encryption import (
    _get_enctype_profile, Key, _HMACMD5
)

from minikerberos.utils import (
    as_str, as_bytes, as_hex
)
...
class KerberosSocket(object):
    __slots__ = (
        'soc_type', 'dst_ip', 'dst_port', 'soc'
    )
    ...
    def sendrecv(self, data, throw=True):
        self.create_soc()
        try:
            self.soc.sendall(struct.pack('>I', len(data)) + data)
            ...
            while True:
                ...
                if total_length is None:
                    response_linearized = response.pop()
                    if len(response_linearized) < 4:
                        response = [response_linearized]
                        continue

                    total_length, = struct.unpack_from(
                        '>I', response_linearized)

                    packet = response_linearized[4:]
            ...
            krb_message = KerberosResponse.load(b''.join(response))
...
class KerbrosComm(object):
    __slots__ = (
        'usercreds', 'ksoc',  'user_ccache',
        'ccache', 'kerberos_session_key',
        'kerberos_TGT', 'kerberos_TGT_encpart',
        'kerberos_TGS', 'kerberos_cipher',
        'kerberos_cipher_type', 'kerberos_key',
        'server_salt'
    )
    ...
    def from_tgt(ksoc, tgt, key):
        ...
        kc.kerberos_session_key = Key(
            kc.kerberos_cipher_type, key['keyvalue']
        )
    ...
    def do_preauth(self, rep):
        ...
        for enc_method in METHOD_DATA.load(rep['e-data']).native:
            ...
            if data_type == PaDataType.ETYPE_INFO or data_type == PaDataType.ETYPE_INFO2:
                ...
                for enc_info in enc_info_list.native:
                    ...
                    logger.debug('Server supports encryption type %s with salt %s' % (
                        EncryptionType(enc_info['etype']).name, enc_info['salt']))
        ...
        self.kerberos_key = Key(
            self.kerberos_cipher.enctype, self.usercreds.get_key_for_enctype(
                supp_enc, salt=self.server_salt))
        enc_timestamp = self.kerberos_cipher.encrypt(
            self.kerberos_key, 1, timestamp, None)
        ...
        kdc_req_body['cname'] = PrincipalName({
            ...
            'name-string': [
                as_str(self.usercreds.username)
            ]
        })
        ...
        kdc_req_body['sname'] = PrincipalName({
            'name-type': NAME_TYPE.PRINCIPAL.value,
            'name-string': [
                'krbtgt', as_str(self.usercreds.domain.upper())
            ]
        })
    ...
    def get_TGT(self, override_etype=None, decrypt_tgt=True):
        ...
        self.kerberos_session_key = Key(
            self.kerberos_cipher.enctype, self.kerberos_TGT_encpart['key']['keyvalue'])
        self.ccache.add_tgt(self.kerberos_TGT,
                            self.kerberos_TGT_encpart, override_pp=True)
    ...
    def get_TGS(self, spn_user, override_etype=None, is_linux=False):
        ...
        ap_req['authenticator'] = EncryptedData({
            'etype': self.kerberos_cipher_type,
            'cipher': authenticator_data_enc
        })
    ...
    def S4U2self(self, user_to_impersonate, supp_enc_methods=SUPPORTED_METHODS):
        ...
        S4UByteArray = NAME_TYPE.PRINCIPAL.value.to_bytes(
        ...
        pa_for_user_enc['userName'] = PrincipalName({
            'name-type': NAME_TYPE.PRINCIPAL.value,
            'name-string': user_to_impersonate.get_principalname()
        })
        ...
        krb_tgs_body = {}
        krb_tgs_body['kdc-options'] = KDCOptions(set([
        ...
        try:
            ...
            if e.errorcode.value == 16:
                logger.error(
                    'S4U2self: Failed to get S4U2self! Error code (16) indicates that delegation is not enabled for this account! Full error: %s' % e)
    ...
    def S4U2proxy(self, s4uself_ticket, spn_user, supp_enc_methods=SUPPORTED_METHODS):
        ...
        pa_pac_opts['padata-value'] = PA_PAC_OPTIONS({
            'value': PA_PAC_OPTIONSTypes(set([
                'resource-based constrained delegation']))}).dump()
        ...
        krb_tgs_body['kdc-options'] = KDCOptions(set([
            'forwardable', 'renewable', 'constrained-delegation', 'canonicalize'
        ]))
        krb_tgs_body['sname'] = PrincipalName({
            'name-type': NAME_TYPE.SRV_INST.value,
            'name-string': spn_user.get_principalname()
        ...
        try:
            ...
            if e.errorcode.value == 16:
                logger.error(
                    'S4U2proxy: Failed to get S4U2proxy! Error code (16) indicates that delegation is not enabled for this account! Full error: %s' % e)
    ...
    def get_something(self, tgs, encTGSRepPart, sessionkey):
        ...
        authenticator_data_enc = cipher.encrypt(
            sessionkey, 11, Authenticator(authenticator_data).dump(), None
        )
