...
from minikerberos.encryption import ARC4
from minikerberos.encryption import hmac_md5
...
class GSSAPIFlags(enum.IntEnum):
...
class GSSMIC_RC4(object):
    __slots__ = (
        'TOK_ID', 'SGN_ALG', 'Filler',
        'SND_SEQ', 'SGN_CKSUM'
    )
...
class GSSWRAP_RC4(object):
    __slots__ = (
        'TOK_ID', 'SGN_ALG', 'Filler', 'SEAL_ALG',
        'SND_SEQ', 'SGN_CKSUM', 'Confounder'
    )
...
class GSSAPI_RC4(object):
    ...
    def GSS_GetMIC(self, data, sequenceNumber, direction='init'):
        ...
        mic = GSSMIC_RC4()
        mic.SND_SEQ = struct.pack('>I', sequenceNumber)
        if direction == 'init':
            mic.SND_SEQ += b'\x00'*4
        else:
            mic.SND_SEQ += b'\xff'*4
        ...
        id = 15
        temp = md5(struct.pack('<I', id) + mic.to_bytes()[:8]).digest()
    ...
    def GSS_Wrap(self, data, seq_num, direction='init', encrypt=True, auth_data=None):
        ...
        token.SND_SEQ = struct.pack('>I', seq_num)

        if direction == 'init':
            token.SND_SEQ += b'\x00'*4
        else:
            token.SND_SEQ += b'\xff'*4
        ...
        id = 13
        Sgn_Cksum = md5(
            struct.pack('<I', id) + token.to_bytes()[:8] +
        ...
        temp.update(struct.pack('>I', seq_num))
        Kcrypt = temp.digest()
        ...
        token.SND_SEQ = ARC4(Kseq).encrypt(token.SND_SEQ)

        if auth_data is not None:
            wrap = GSSWRAP_RC4.from_bytes(
                auth_data[8 + len(GSS_WRAP_HEADER):])
            ...
            snd_seq = ARC4(temp.digest()).encrypt(wrap.SND_SEQ)
...
class GSSMIC(object):
    ...
    def from_buffer(buff):
        ...
        m.Flags = struct.unpack('B', buff.read(1)[0])
        m.Filler = buff.read(5)
        m.SND_SEQ, = struct.unpack('>Q', buff.read(8))
        m.SGN_CKSUM = buff.read()  # should know the size based on the algo!
        return m

    def to_bytes(self):
        t = self.TOK_ID
        t += struct.pack('B', self.Flags)
        t += self.Filler
        t += struct.pack('>Q', self.SND_SEQ)
        if self.SGN_CKSUM is not None:
            t += self.SGN_CKSUM
...
class GSSWrapToken(object):
    ...
    def from_buffer(buff):
        ...
        m.EC, = struct.unpack('>H', buff.read(2))
        m.RRC, = struct.unpack('>H', buff.read(2))
        m.SND_SEQ, = struct.unpack('>Q', buff.read(8))
        return m
    ...
    def to_bytes(self):
        ...
        t += struct.pack('B', self.Flags)
        t += self.Filler
        t += struct.pack('>H', self.EC)
        t += struct.pack('>H', self.RRC)
        t += struct.pack('>Q', self.SND_SEQ)
...
class GSSAPI_AES(object):
    __slots__ = (
        'session_key', 'checksum_profile', 'cipher_type', 'cipher'
    )
    ...
    def GSS_GetMIC(self, data, seq_num):
        ...
        m.checksum = checksum_profile.checksum(
            self.session_key,
            KG_USAGE.INITIATOR_SIGN.value,
            data + m.to_bytes()[:16]
        )
...
if __name__ == '__main__':
    ...
    cipherText_2 = h(
        ...
        '46b5d910417142dc7b7ba7ded76cddc4acf9bf7ed440'
    )

    session_key = encryption.Key(
        encryption.Enctype.AES256,
        h('3e242e91996aadd513ecb1bc2369e44183e08e08c51550fa4b681e77f75ed8e1')
    )

    data = h(
        '810e00001a204de2d64fd111a3da0000f875ae0d1c4500003400000034000'
        '0000080400500000000000000000000000000000000000000000000000000'
    ...
    ret1 = h(
        '4ae025fa2a9c337c75c024d9d8f0186c75a4a9060e2a40a9ad024317bf5df6a'
        '86cb4a764a9ca36843f8fa4f99c03e2bde46f5a29aafc83dacdf9f0a5677446b'
        '5d910417142dc7b7ba7ded76cddc4acf9bf7ed440'
    )

    ret2 = h(
        '050406ff000c001c000000000000000008cb9850e5701f2f9285dad6463c'
        'a8d0e365d4f1700f3d054e242ebcde2f3146ddd411a627af74860880ed78'
        'd6196dde3f3fb23eeea650bc'
    )
